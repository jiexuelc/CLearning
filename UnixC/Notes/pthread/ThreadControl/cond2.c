...
int main(void)
{
	...
	pthread_t tid;
	if ((error = pthread_create (&tid, &attr, thread_producer, "Producer1")) != 0) {
		fprintf (stderr, "pthread_create: %s\n", strerror (error));
		return -1;
	}	
	if ((error = pthread_create (&tid, &attr, thread_producer, "Producer2")) != 0) {
		fprintf (stderr, "pthread_create: %s\n", strerror (error));
		return -1;
	}
	
	if ((error = pthread_create (&tid, &attr, thread_customer, "Customer1")) != 0) {
		fprintf (stderr, "pthread_create: %s\n", strerror (error));
		return -1;
	}
	if ((error = pthread_create (&tid, &attr, thread_customer, "Customer2")) != 0) {
		fprintf (stderr, "pthread_create: %s\n", strerror (error));
		return -1;
	}
